...
class NeuralNetwork:
    ...
    def __init__(
        ...
        self.__amount_hidden_layer_nodes = int((amount_input_nodes + len(targets)) / 4) # TODO: how to find out best size?
    ...
    def __initialize_layers(self):
        ...
        for _ in range(self.__amount_hidden_layers): self.__layers.append([0 for _ in range(self.__amount_hidden_layer_nodes)])
        self.__layers.append([0 for _ in range(len(self.__targets))])

    def __reset_layers(self):
        for i in range(self.__amount_hidden_layers): self.__layers[i + 1] = [0 for _ in range(self.__amount_hidden_layer_nodes)]
        self.__layers[len(self.__layers) - 1] = [0 for _ in range(len(self.__targets))]
